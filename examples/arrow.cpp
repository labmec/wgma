                     ...
                     const SimData &simdata)
{
  ...
  */
  ...
  for(int im_left = 0; im_left < nmodes_left.size(); im_left++){
    ...
    for(int im_right = 0; im_right < nmodes_right.size(); im_right++){
      ...
      if(simdata.compute_reflection_norm){
        ...
        ost << simdata.lambda<<','
            <<ref.real()<<ref_sign<<std::abs(ref.imag())<<'j'<<','
            <<trans.real()<<trans_sign<<std::abs(trans.imag())<<'j'<<std::endl;
      }
  ...
  if(match_an){wgma::cmeshtools::RemovePeriodicity(match_data.cmesh);}
}
