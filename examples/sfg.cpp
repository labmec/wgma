/**
   ...
   This target is used to model sum frequency generation on
   non linear materials.
   Two linear simulations are computed (pump and signal field)
   and the polarisation of the non-linear material based on the
   computed fields is used as a source for the simulation at the SFG frequency.
***/
...
struct SimData{
  //!.msh mesh
  std::string meshfile;
  //!wavelength vector of pump field
  TPZVec<STATE> pump_vec;
  //!wavelength vector of signal field
  TPZVec<STATE> sign_vec;
  //!wavelength vector of sfg field
  TPZVec<STATE> sfg_vec;
  //!map of refractive indices
  std::map<std::string,n_func> ref_index_map;
  //!map of refractive indices for current simulation point
  std::map<std::string,CSTATE> refractive_indices;
  //!wavelength of current simulation point
  STATE lambda{0};
  //!pairs of mode index/coefficient to be used as source for current simulation point
  ...
  std::map<std::string,int> h_regions;
  ...
  REAL solver_tol;
  //!pairs of mode index/coefficient to be used as source for pump simulations
  std::vector<std::pair<int,double>> pump_coeffs;
  //!pairs of mode index/coefficient to be used as source for signal simulations
  std::vector<std::pair<int,double>> sign_coeffs;
  //!materials in which polarisation is to be computed
  TPZVec<std::string> polarisation_regions = {};
...
//!needed data to be exported from linear simulations
struct ScattData{
...
//! Computes a linear simulation
TPZAutoPointer<ScattData>
ComputeLinearSimulation(TPZAutoPointer<TPZGeoMesh> gmesh,
                        const TPZVec<std::map<std::string, int>> &gmshmats,
...
//! Computes the non-linear simulation
void ComputeSFGSimulation(TPZAutoPointer<ScattData> &pump_data,
                          ...
                          SimData &simdata, std::map<int,int> &split_mats,
                          TPZFMatrix<CSTATE> &last_sol, const int iwl);
//! Compute plane wave solutions for a rectangular homogeneous waveguide port
TPZAutoPointer<ModalData>
ComputePlaneWaveSolutions(
  TPZAutoPointer<TPZGeoMesh> gmesh,
  const TPZVec<std::map<std::string, int>> &gmshmats,
...
int main(int argc, char *argv[]) {
  ...
  // {
  //   constexpr int nref{0};
  //   for(int iref = 0; iref < nref; iref++)
  //   for(auto gel : gmesh->ElementVec()){
  //     TPZManVector<TPZGeoEl*,20> sons;
  //     if(gel && gel->IsGeoBlendEl() && gel->NSubElements() == 0){
  //       gel->Divide(sons);
  //     }
  //   }
  // }
  ...
  RefineRegions(gmesh, gmshmats, simdata.h_regions);
  ...
  TPZFMatrix<CSTATE> last_sol_pump, last_sol_sign, last_sol_sfg;
  //number of wavelength points
  const int nwl_pump = simdata.pump_vec.size();
  const int nwl_sign = simdata.sign_vec.size();
  const auto orig_prefix = simdata.prefix;
  for(int ipump = 0; ipump < nwl_pump; ipump++){
    auto timer_begin = std::chrono::high_resolution_clock::now();
    const auto wlpump = simdata.pump_vec[ipump];
    ...
    for(int isign = 0; isign < nwl_sign; isign++){
      const auto wlsign = simdata.sign_vec[isign];
      ...
      signmesh->ComputeNodElCon();
      signmesh->CleanUpUnconnectedNodes();
    }
    ...
    //removing restrictions
    auto pumpmesh = pumpdata->cmesh;
    wgma::cmeshtools::RemovePeriodicity(pumpmesh);
    pumpmesh->ComputeNodElCon();
    pumpmesh->CleanUpUnconnectedNodes();
  }
  return 0;
}
...
SimData ReadSimData(const std::string &dataname){
  ...
  tmpvec_wl = data["pump_wl"].get<std::vector<STATE>>();
  for(auto wl : tmpvec_wl){sd.pump_vec.push_back(wl);}
  tmpvec_wl = data["sign_wl"].get<std::vector<STATE>>();
  for(auto wl : tmpvec_wl){sd.sign_vec.push_back(wl);}
  //now we get min and max lambda and check if we have refractive index for all of them

  {
    ...
    STATE wl = *std::min_element(begin(sd.pump_vec), end(sd.pump_vec));
    wl = std::min(wl,*std::min_element(begin(sd.sign_vec), end(sd.sign_vec)));
    TestWavelength(wl);
    wl = *std::max_element(begin(sd.pump_vec), end(sd.pump_vec));
    wl = std::max(wl,*std::max_element(begin(sd.sign_vec), end(sd.sign_vec)));
    TestWavelength(wl);
  }
  ...
  sd.custom_periodic_bcs_port_out = data.value("periodic_bcs_port_out",
                                         std::map<std::string,std::string>{});
  sd.h_regions = data.value("refine_regions", std::map<std::string,int> {});
  ...
  {
    tmpvec_str = data["polarisation_regions"].get<std::vector<std::string>>();
    for(auto mat : tmpvec_str){sd.polarisation_regions.push_back(mat);}
  }
  ...
  sd.pump_coeffs = data["pump_coeffs"].get<std::vector<std::pair<int,double>>>();
  sd.sign_coeffs = data["sign_coeffs"].get<std::vector<std::pair<int,double>>>();
  ...
  const std::string &suffix)
{
  ...
  int nEigenpairs = 2*(max_k*2+1)*(max_k*2+1);
   ...
   ******************************/
  const int krylovDim =
    nEigenpairs < 20 ? nEigenpairs*5 : (int)std::ceil(3*nEigenpairs);
  auto solver =
    wgma::wganalysis::SetupSolver(target, nEigenpairs, sortingRule,
                                  usingSLEPC,krylovDim,verbose);
...
void AdjustPOrder(TPZAutoPointer<TPZCompMesh> scatt_mesh,
                  const TPZVec<std::map<std::string, int>> &gmshmats,
                  const std::map<std::string,int> &p_regions);

void AdjustRefinedEdges(TPZAutoPointer<TPZCompMesh> scatt_mesh,
                        const TPZVec<std::map<std::string, int>> &gmshmats,
                        const std::map<std::string,int> &h_regions);
...
void AdjustRefinedEdges(TPZAutoPointer<TPZCompMesh> scatt_mesh,
  ...
  //just to avoid iterating through the same element over and over
  std::set<int64_t> refined_els;
  for(auto [name,nref] : h_regions){
    bool found{false};
    int matid{-1};
    for(auto idim = 0; idim < 3 && found==false; idim++){
      auto &mats_dim = gmshmats[idim];
    ...
    for(auto gel : gmesh->ElementVec()){
      ...
      for(auto is = nnodes; is < nsides-1; is++){
        ...
        while(neigh && neigh != gelside){
          ...
          if(cel){
            if(cel->Mesh() == scatt_mesh.operator->()){
              if(refined_els.count(cel->Index())==0){
                ...
                for(auto icon = 0; icon < ncon; icon++){
                  ...
                  if(c.Order()>0 && c.NShape() > 0){
                    ...
                    scatt_mesh->Block().Set(seq,nshape);
                  }
                }//for connects
                ...
                )
{
  ...
  */
  ...
  if(src_ids.size()){
    cmesh =
      wgma::scattering::CMeshScattering3DPeriodicVolSource(gmesh, pOrder, scatt_data,
                                                           el_map,
                                                           src_ids,
