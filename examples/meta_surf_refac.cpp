...
void ComputePlaneWaveSolutions(TPZAutoPointer<ModalData> portdata,
                              const SimData& simdata,
                              const std::string &suffix);
//! Creates mesh and analysis instance for computing modal analysis in ports
TPZAutoPointer<ModalData>
SetupModalAnalysis(
  TPZAutoPointer<TPZGeoMesh> gmesh,
  const TPZVec<std::map<std::string, int>> &gmshmats,
...
//! Computes modal anlaysis for a given wavelength
void ComputeModalAnalysis(TPZAutoPointer<ModalData> portdata,
                          const SimData& simdata,
                          const std::string &suffix);
...
int main(int argc, char *argv[]) {
  ...
  TPZAutoPointer<ModalData> modal_an_in{nullptr};
  if(simdata.custom_periodic_bcs_port_in.size() == 0){
    modal_an_in = SetupPlaneWaveSolutions(gmesh,gmshmats,simdata, periodic_els,
                                         simdata.mats_port_in,
                                         simdata.max_k_in,"_port_in");
  }else{
    modal_an_in = SetupModalAnalysis(gmesh,gmshmats,simdata, periodic_els,
                                     simdata.mats_port_in,
                                     simdata.max_k_in,"_port_in");
  }
  ...
  if(simdata.mats_port_out.size()){
    if(simdata.custom_periodic_bcs_port_out.size() == 0){
      ...
      modal_an_out = SetupModalAnalysis(gmesh,gmshmats,simdata, periodic_els,
                                        simdata.mats_port_out,
                                        simdata.max_k_out,"_port_out");
    }
  }
  ...
  for(int iwl = 0; iwl < nwl_pts; iwl++){
    ...
    if(simdata.custom_periodic_bcs_port_in.size() == 0){
      ...
      ComputeModalAnalysis(modal_an_in, simdata, "_port_in");
    }
    ...
    if(simdata.mats_port_out.size()){
      if(simdata.custom_periodic_bcs_port_out.size() == 0){
        ...
        ComputeModalAnalysis(modal_an_out, simdata, "_port_out");
      }
    }
  ...
  const std::string &suffix)
{
  ...
  STATE max_n{0};
  for(auto [matid, er, ur] : physical_data.matinfovec){
  ...
  const auto &scale = simdata.scale;
  const bool verbose = simdata.eigen_verbose;
  auto modal_cmesh = wgma::wganalysis::CMeshWgma2DPeriodic(gmesh,p_order,physical_data,
                                                           el_map,
                          ...
                          const std::string &suffix)
{
  ...
  for(auto [id,matptr] : cmesh_mf->MaterialVec()){
    ...
    for(auto [matid, matname] : physical_data.matnamevec){
      if(id == matid){
        found = true;
        auto mat =
          dynamic_cast<TPZWgma*>(matptr);
        if(!mat){DebugStop();}
        const auto suffix_length = suffix.size();
        const auto name = matname.substr(0,matname.length()-suffix_length);
        const CSTATE n = simdata.refractive_indices.at(name);
        mat->SetWavelength(simdata.lambda);
        mat->SetPermittivity(n*n);
        break;
      }
    }
    if(!found){DebugStop();}
  ...
  auto an =
    TPZAutoPointerDynamicCast<wgma::wganalysis::Wgma2D>(portdata->an);
  ...
  const auto nEigenpairs = betavec.size();
