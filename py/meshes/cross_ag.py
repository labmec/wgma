...
def create_cross_mesh(w_domain, h_domain, w_cross, l_cross, d_cross,
    ...
    h_cross = d_sub-d_cross
    # air upper half domain
    air = BoxData(-w_domain/2, -w_domain/2, d_sub, w_domain, w_domain, h_air)
    create_box(air)

    # silver
    ag = BoxData(-w_domain/2, -w_domain/2, 0, w_domain, w_domain, d_sub)
    create_box(ag)
    # cross 1
    ...
    ag_air_ma = get_boundary_in_dir(vol_domains, 'zm')
    air_top_ma = get_boundary_in_dir(vol_domains, 'zp')
    # we need to split ag_ma into air + ag
    ag_bot_ma = list(set(set(ag_air_ma) & set(ag_bnds)))
    air_bot_ma = list(set([t for t in ag_air_ma if t not in ag_bnds]))
    ...
    xp_bnd_port_out = get_boundary_in_dir([(2, t) for t in ag_air_ma], 'xp')
    xm_bnd_port_out = get_boundary_in_dir([(2, t) for t in ag_air_ma], 'xm')
    yp_bnd_port_out = get_boundary_in_dir([(2, t) for t in ag_air_ma], 'yp')
    ym_bnd_port_out = get_boundary_in_dir([(2, t) for t in ag_air_ma], 'ym')
    ...
    gmsh.model.mesh.field.set_number(field_ct, "SizeMin", min_el/4)
    ...
    domain_regions = {
        ...
        "Ag_port_out": ag_bot_ma,
        "air_port_out": air_bot_ma,
...
create_cross_mesh(w_domain, h_domain, w_cross, l_cross,
                  d_cross, d_sub, el_ag, el_air, filename)
