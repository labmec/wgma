...
def gen_script_and_run(data, scriptname, prefix):
    ...
    p = subprocess.Popen('test -f ../arrow_wg',
                         stdout=subprocess.PIPE, shell=True)
    status = p.wait()
    # found executable
    if status == 0:
        # run program
        os.system('cd .. && ./arrow_wg scripts/'+scriptname)
...
scriptname = "arrow_wg_validation.json"
prefix = "res_arrow_wg_validation/arrow"
data["meshfile"] = "meshes/arrow_validation.msh"
